 ...
 */
MazeGenerator::MazeGenerator(S21Matrix<char>& verticalMatrix,
                             S21Matrix<char>& horizontalMatrix, int rows,
                             int cols)
...
void MazeGenerator::assignUniqueSets() {
  for (int i = 0; i < cols_; i++)
    if (!mazeLine_[i]) mazeLine_[i] = setCounter_++;
}
 ...
 */
char MazeGenerator::random() { return rand() % 2 ? '1' : '0'; }
...
void MazeGenerator::mergeSet(int index) {
  ...
  for (int i = 0; i < cols_; i++)
    if (mazeLine_[i] == set) mazeLine_[i] = mazeLine_[index];
}
...
int MazeGenerator::countHorizontalWalls(int set, int row) {
  ...
  for (int i = 0; i < cols_; i++)
    if (mazeLine_[i] == set && horizontalMatrix_(row, i) == '0') counter++;
...
void MazeGenerator::updateLine(int row) {
  for (int i = 0; i < cols_; i++)
    if (horizontalMatrix_(row, i) == '1') mazeLine_[i] = 0;
}
