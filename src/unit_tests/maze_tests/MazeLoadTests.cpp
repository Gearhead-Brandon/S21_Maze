...
TEST_F(MazeLoadTests, MazeLoadTest) {
  ...
  std::vector<Line> expectedLines = {
      {0, 1, 500, 1},       {1, 500, 1, 0},       {499, 499, 0, 499},
      {499, 0, 499, 499},   {0, 125, 125, 125},   {250, 125, 375, 125},
      {125, 125, 125, 250}, {375, 125, 375, 250}, {250, 250, 375, 250},
      {0, 375, 125, 375},   {250, 250, 250, 375}, {125, 375, 250, 375},
      {375, 375, 500, 375}};
...
TEST_F(MazeLoadTests, IncorrectMazeSize) {
  // Act
  OpResult result =
      maze_.load("test_files/maze/incorrect_size/IS_rows_less_0.txt");
  std::vector<Line> lines = std::move(maze_.get(500, 500));
...
TEST_F(MazeLoadTests, InvalidMazeData) {
  // Act
  OpResult result =
      maze_.load("test_files/maze/invalid_maze_data/invalid_data_v.txt");
  std::vector<Line> lines = std::move(maze_.get(500, 500));
